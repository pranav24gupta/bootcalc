...
build:all
	rm mycalc/boot/grub/ -r -f
	mkdir mycalc/boot/grub/
	echo set default=0 >> mycalc/boot/grub/grub.cfg
	echo set timeout=0 >> mycalc/boot/grub/grub.cfg
	echo menuentry "mycalc" { >> mycalc/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> mycalc/boot/grub/grub.cfg
	echo } >> mycalc/boot/grub/grub.cfg

	grub-mkrescue -o mycalc.iso mycalc/
